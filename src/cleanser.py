...
def reader(filename, filepath):
    ...
    if filetype == "txt" or filetype == "log":
        ...
        write(data_csv, filename, filepath)
...
def write(lineList, filename, filepath):
    ...
    if(filetype == "txt"):
        ...
        file = os.path.join(filepath, filename[0: len(filename)-4] + "temp.txt")
        # Unwanted characters
    ...
    #Cleanse .log file
        ...
        # This is done to allow multiple testing
        file = os.path.join(filepath, filename[0: len(filename)-4] + "temp.log")
        with open(file, "w") as outputFile:
            for line in lineList:
                if line.strip() != "":
                    outputFile.write(line + "\n")
    ...
    #Cleanse .csv file
        ...
        # This is done to allow multiple testing
        file = os.path.join(filepath, filename[0: len(filename)-4] + "temp.csv")
        with open(file, "w") as outputFile:
            for line in lineList:
                for i in line:
                    if i.strip() != "":
