...
class SPLUNKInterface:
    ...
    def connect_analyst_with_token(self, token, username="", password=""):
        try:
            if (self.connect_client(username,password)):
                self.session_token = token[7:]
                print("token for new session is:")
                ...
                self.splunkClient = client.Service(token="atg232342aa34324a")
